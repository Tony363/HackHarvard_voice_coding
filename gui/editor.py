...
class myEditor(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.completer.setModel(self.modelFromFile(
            self.root + '/resources/wordlist.txt'))
...
#        self.editor.setTabStopWidth(20)
        ...
        self.createActions()
        # Syntax Highlighter ...
        print("document", type(self.editor.document()))
        ...
        self.uncommentAct.setIcon(
            QIcon.fromTheme(self.root + "/icons/uncomment"))
        ...
        self.commentBlockAct.setIcon(
            QIcon.fromTheme(self.root + "/icons/commentBlock"))
        ...
        self.uncommentBlockAct.setIcon(
            QIcon.fromTheme(self.root + "/icons/uncommentBlock"))
        tb.addAction(self.uncommentBlockAct)
        # color chooser
        tb.addSeparator()
        tb.addAction(QIcon.fromTheme(self.root + "/icons/color1"),
                     "insert QColor", self.insertColor)
        tb.addSeparator()
        tb.addAction(QIcon.fromTheme("preferences-color"),
        ...
        # about buttons
        tb.addSeparator()
        tb.addAction(QIcon.fromTheme(self.root + "/icons/info2"),
        ...
        # end toolbar
        self.indentAct = QAction(QIcon.fromTheme(self.root + "/icons/format-indent-more"),
                                 "indent more", self, triggered=self.indentLine, shortcut="F8")
        self.indentLessAct = QAction(QIcon.fromTheme(
            self.root + "/icons/format-indent-less"), "indent less", self, triggered=self.indentLessLine, shortcut="F9")
        ...
        tbf.addAction(QAction(QIcon.fromTheme("document-properties"),
                      "check && reindent Text", self, triggered=self.reindentText))
        ...
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-undo'), "Undo", self, triggered=self.editor.undo, shortcut="Ctrl+u"))
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-redo'), "Redo", self, triggered=self.editor.redo, shortcut="Shift+Ctrl+u"))
        editmenu.addSeparator()
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-copy'), "Copy", self, triggered=self.editor.copy, shortcut="Ctrl+c"))
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-cut'), "Cut", self, triggered=self.editor.cut, shortcut="Ctrl+x"))
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-paste'), "Paste", self, triggered=self.editor.paste, shortcut="Ctrl+v"))
        editmenu.addAction(QAction(QIcon.fromTheme(
            'edit-delete'), "Delete", self, triggered=self.editor.cut, shortcut="Del"))
        editmenu.addSeparator()
        editmenu.addAction(QAction(QIcon.fromTheme('edit-select-all'), "Select All",
                           self, triggered=self.editor.selectAll, shortcut="Ctrl+a"))
        editmenu.addSeparator()
...
#        self.installEventFilter(self)
    ...
    def contextMenuRequested(self, point):
        ...
        if not self.editor.textCursor().selectedText() == "":
            ...
            cmenu.addSeparator()
        cmenu.addAction(QIcon.fromTheme("zeal"),
                        "show help with 'zeal'", self.showZeal)
        cmenu.addAction(QIcon.fromTheme("gtk-find-"),
                        "find this (F10)", self.findNextWord)
        cmenu.addSeparator()
        ...
        cmenu.addSeparator()
        cmenu.addAction(QIcon.fromTheme("preferences-color"),
                        "insert QColor", self.insertColor)
    ...
    def replaceThis(self):
        ...
        text = QInputDialog.getText(
            self, "replace with", "replace '" + rtext + "' with:", QLineEdit.Normal, "")
...
#            self.editor.find(ot)
    ...
    def dataReady(self):
        ...
        if self.mylabel.find("line", QTextDocument.FindWholeWords):
            t = self.mylabel.toPlainText().partition(
                "line")[2].partition("\n")[0].lstrip()
            if t.find(",", 0):
                tr = t.partition(",")[0]
            else:
    ...
    def gotoErrorLine(self, ln):
        if ln.isalnum:
            ...
            if t != 0:
                linecursor = QTextCursor(
                    self.editor.document().findBlockByLineNumber(t-1))
                self.editor.moveCursor(QTextCursor.End)
                self.editor.setTextCursor(linecursor)
                self.editor.moveCursor(
        ...
        if path:
            ...
            if inFile.open(QFile.ReadWrite | QFile.Text):
                ...
                self.statusBar().showMessage("File '" + path +
                                             "' loaded succesfully & bookmarks added & backup created ('" + self.filename + "_backup" + "')")
...
#                self.completer.setModel(self.modelFromFile(self.root + '/resources/wordlist.txt'))
    ...
    def readData(self, cmd):
        self.mylabel.clear()
        dname = QFileInfo(self.filename).filePath().replace(
    ...
    def createTrayIcon(self):
        if not QSystemTrayIcon.isSystemTrayAvailable():
            ...
            self.trayIconMenu.addAction(QAction(QIcon.fromTheme(
                "applications-python"), "about PyEdit", self, triggered=self.about))
            self.trayIconMenu.addSeparator()
            self.trayIconMenu.addAction(QAction(QIcon.fromTheme(
                "application-exit"), "Exit", self, triggered=self.handleQuit))
